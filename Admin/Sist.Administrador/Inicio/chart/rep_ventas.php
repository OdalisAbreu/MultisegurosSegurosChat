...
$wFecha = "seguro_transacciones.fecha >= '$fDesde' AND seguro_transacciones.fecha < '$fHasta 23:59:59'";
...
$queryV = mysql_query("
				SELECT 
					SUM(seguro_transacciones.monto) AS total,
					SUM(seguro_transacciones.ganancia1) AS gantotal, 
					fecharep
				FROM 
					seguro_transacciones 
				LEFT JOIN 
					seguro_transacciones_reversos 
				ON 
					seguro_transacciones_reversos.id_trans = seguro_transacciones.id
				WHERE 
					$wFecha
					AND seguro_transacciones_reversos.id_trans IS NULL  -- Filtra las transacciones que no están en reversos
				GROUP BY 
					fecharep;
			");
